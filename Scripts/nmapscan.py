...
def analyze(data):
    try:
        ...
        for ip in data['scan']:
            #print('IP Adress: ' + ip)
            services['os'] = check_os(data['scan'][ip])
            for port in data['scan'][ip]['tcp']:
                #print(
                #    str(port) +
                #    ' ' + 
                #    data['scan'][ip]['tcp'][port]['state'] +
                #    ' ' + 
                #    data['scan'][ip]['tcp'][port]['name']
                #)
                
                services[port] = {
                    "state": data['scan'][ip]['tcp'][port]['state'], 
                    "product": data['scan'][ip]['tcp'][port]['product'], 
                    "version": data['scan'][ip]['tcp'][port]['version']
                }
